...
Definition guess (p: Person): Flip :=
  match p with
    | person strategy flip => strategy flip
  end.
...
Definition success (p1: Person) (p2: Person): Prop :=
  match p1, p2 with
    | person s1 f1, person s2 f2 => if orb (flip_eq (s1 f1) f2) (flip_eq (s2 f2) f1) then True else False
  end.
